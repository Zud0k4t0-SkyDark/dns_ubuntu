...
function org(){ 
	...
	echo -en "Ip:\n==> " && read ip ; ${ip:=$ip_new_use} > /dev/null 2>&1
	echo $ip
	echo -e $name
	echo -en "Nombre:\n==> " && read name ; ${name:='site'} > /dev/null 2>&1
	echo -e $name
	ip_a=$(echo $cidr | grep -oP "(\d{1,}.){1,}([/$])" | grep -oP "(\.\d{1,3}[\/$])" | grep -oP "([^.].*[^/])")
	cidr_With_Cero=$(echo  "${cidr//$ip_a/"0"}")
	echo -en "Direccion de Red:\t($cidr_With_Cero)\n==> " && read d_red
	
#Grepear por la ip
	packages
	ip_se=$(echo "$ip_cero" | grep -oP "\d{1,3}.\d{1,3}.\d")
	echo $ip_se
	rev_ip=$(echo $ip_se | awk -F . '{print $3"."$2"."$1}')
	echo $rev_ip
	interfaces
	named 1
	zonas
...
function interfaces(){
	...
	echo -e "\tallow-query { localhost; $cidr_With_Cero; }; " >> named.conf.options
...
function validar(){
	...
	if [[ $res == '1' ]];then
		...
		sudo named-checkzone db.$ip_se.in-addr.arpa /etc/bind/zonas/db.$ip_se
		bind9_res_sta
		internet
	fi
}
...
function zonas(){
	...
	echo -e "zone \"$ip_se.in-addr.arpa\" {" >> named.conf.local
	echo -e "\ttype master; " >> named.conf.local
	echo -e "\tfile \"/etc/bind/zonas/db.$rev_ip\"; " >> named.conf.local
	echo -e "}; " >> named.conf.local
}
